...
*  their own connections and so on. Every time the peer receives a query, a message is sent to the server to look for queryhits,
*  if there is one, it gets routed back to the that queried.
*
*******************************************************************/
	...
	if ( this->state() == passive)
	{
		...
		else if(msg.port() == q_in) //keep propagating the query
		{
			...
			if(msgIdList.find(msgId) == msgIdList.end() && msgIdList2.find(msgId) == msgIdList2.end() && TTL > 0 && qhit != 1){
					localQuery.push(buildNewMessage(query, qhit, msgId, 0, myId, 0)); //build message to be outputted to server
					set<int>::iterator connection;
					for(connection = connectionlist.begin();connection != connectionlist.end();connection++){
			 ...
			 * therefore sends a message to the RandomSurfer Model*/
				...
				if((*primary).find(msgId) != (*primary).end()){
					...
					if(VERBOSE) cout << "queryhit being sent to RandomSurfer"<<endl;
				}
