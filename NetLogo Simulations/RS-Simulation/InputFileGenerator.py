...
docInput = open("MultiPeer.txt", "w")
friendInput = open ("friendList.txt", "w")
                


for j in range(size): #the first element is a number indicating how many times the second is repeated
    a = docsdistribution[j].split(",")
    for i in a:
        docInput.write(i + "\n")
    docInput.write("end\n") #write other element replacing the {num} with the appropriate integer
